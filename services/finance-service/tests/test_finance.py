...
import pytest
from core.handlers import handle_booking_approved, handle_contract_activated
...
def _create_payment(*, company_id: str, source: str = Payment.Source.CONTRACT, **overrides: object) -> Payment:
    payload = {
...
def test_payment_detail_patch_marks_paid_and_publishes_event() -> None:
    ...
    with patch("core.views.transaction.on_commit") as on_commit, patch(
        "core.handlers.event_bus.publish"
    ) as publish:
...
def test_dashboard_and_reports_are_scoped() -> None:
    cid1 = uuid.uuid4()
    _create_payment(company_id=str(cid1), source=Payment.Source.CONTRACT, amount="100", status=Payment.Status.PAID)
    _create_payment(company_id=str(cid1), source=Payment.Source.CONTRACT, amount="50", status=Payment.Status.PENDING)
    _create_payment(company_id=str(cid1), source=Payment.Source.BOOKING, amount="25", status=Payment.Status.OVERDUE)
    other = uuid.uuid4()
    _create_payment(company_id=str(other), source=Payment.Source.BOOKING, amount="12", status=Payment.Status.PENDING)
...
def test_consumer_command_wires_expected_routing_keys() -> None:
    with patch("core.management.commands.consume_finance_events.event_bus.subscribe") as subscribe:
        call_command("consume_finance_events", rabbitmq_url="amqp://rabbit", queue="finance.unit-test")
